...
layout = dbc.Container([
    dbc.Row([
        ...
        ], md=12, id='layout_operacoes', style={"height": "100%", "maxHeight": "36rem", "overflow-y": "auto"})
    ], className='g-2 my-auto')
],fluid=True),

# Callbacks =======================
#call back auxiliar para ativar o layout das operações
@app.callback(
    Output('layout_operacoes', 'children'),
    Input('layout_data', 'data')
)
def func_auxiliar(data):
    return data

#callback que realiza alterações nos ativos das operações
@app.callback(
...
def func_modal(n1, n2, book_data, event, ativo, open, radio, preco, periodo, vol):
    ...
    # 2. Salvando ativo
        ...
        if None in [ativo, preco, vol] and open:
            ...
            ativo = ativo.upper()
            if verificar_existencia_ativo(ativo+'.SA'):
                exchange = '.SA'
                preco = round(preco, 2)
                ...
                df_book_data.to_csv(r'assets\book_data.csv')
