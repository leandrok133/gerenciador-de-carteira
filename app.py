...
try:    # caso exista, ler infos
    df_book = pd.read_csv(book_data_path, index_col=0)
...
try:
    df_historical_data = pd.read_csv(historical_data_path, index_col=0)
except:
...
def atualizar_databases(book_data, historical_data):
    ...
    df_historical.to_csv(historical_data_path)
